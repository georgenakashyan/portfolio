...
| viewport={{ once: true }} for scroll reveals | 05-02 | Animations play only on first view |

**Issues Resolved:**

- Page transition animation repeating on hydration (fixed with FrozenRouter pattern)
- H1UserAgentFontSizeInSection Lighthouse warning (fixed with :where(h1) rule)
- Focus indicators not visible on all backgrounds (fixed with two-color ring)

**Issues Deferred:**

- Light/dark mode toggle (deferred to v2)
- Code block syntax highlighting (deferred to v2)

**Technical Debt Incurred:**

- LayoutRouterContext dependency on Next.js internal API (documented fallback plan in STATE.md)
